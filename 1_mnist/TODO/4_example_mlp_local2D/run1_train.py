...
class Net(nn.Module):
    def __init__(self):
        ...
        # self.w1 = Variable(torch.randn(27, 27, 64, 1, 3, 3))  # outH x outW x outC x inC x kH x kW
        # self.b1 = Variable(torch.randn(64, 27, 27))  # outC x outH x outW

        ## arguments: kH, kW, strideH, strideW, padH, padW, inH, inW, outH, outW
        self.local1 = nn.SpatialConvolutionLocal(3, 3, 1, 1, 0, 0, 28, 28, 27, 27)
    ...
    def forward(self, x):
        x = x.view(-1, 1, 28, 28)
        # -----
        x = self.local1(x) # N x outC x outH x outW
        x = F.max_pool2d(x, 2)
        x = F.relu(x)
        # -----
        # x = self.conv2(x)
