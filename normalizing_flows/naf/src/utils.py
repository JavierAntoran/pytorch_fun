...
def eval_cos_distance(x1, x2):
    x1n = np.linalg.norm(x1, 1).reshape(-1, 1)
    x2n = np.linalg.norm(x2, 1).reshape(-1, 1)
...
def softmax(A):
    # expA = exp( A );
    # O = expA ./ ( ones(size(A,1),1) * sum(expA,1));
    m = np.max(A, axis=1, keepdims=True)
    expA = np.exp(A - m)
    O = expA / np.sum(expA, axis=1, keepdims=True)
    O = np.clip(O, 1e-9,np.inf)
    return O
